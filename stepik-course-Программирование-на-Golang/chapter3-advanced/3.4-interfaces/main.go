...
const UnknownOperation = "неизвестная операция"

func printf(value interface{}) {
	switch val := value.(type) {
	default:
		fmt.Printf("value=%v: %T\n", val, val)
	}
}
func printVal(num float64) {
	fmt.Printf("%.4f\n", num)
}
func main() {
	//1
	...
	//fmt, и os - не удаляйте их из импорта. Скорее всего, вам понадобится пакет "fmt", но вы можете использовать
	...
	//будут float64. Третье значение в идеальном случае будет строкой, которая может иметь значения:
	...
	//первого значения увидели, что оно содержит ошибку - печатайте сообщение об ошибке и завершайте работу программы,
	//проверка остальных аргументов значения уже не имеет, а проверяющая система воспримет 2 сообщения
	//об ошибке как нарушение условия выполнения задания.
	//Удачи!
	...
	oper, ok := operation.(string)
	if !ok {
		fmt.Println(UnknownOperation)
		return
	}
	switch oper {
	case "+":
	...
	//0 или 1 (порядок 0/1 случайный). Ваша задача считать эту строку любым возможным способом и
	//создать на основе этой строки объект объявленного вами на первом этапе типа: надеюсь,
